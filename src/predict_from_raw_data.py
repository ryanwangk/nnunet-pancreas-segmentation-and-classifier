...
class nnUNetPredictor(object):
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                if ofile is not None:
                    ...
                    self.current_case = os.path.basename(ofile) if ofile else f"case_{len(r)}"
                else:
                    print(f'\nPredicting image of shape {data.shape}:')
                    self.current_case = f"case_{len(r)}"
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor):
        ...
        seg_logits, cls_logits = self.network(x)
        ...
        if mirror_axes is not None:
            ...
            for axes in axes_combinations:
                seg_flipped, cls_flipped = self.network(torch.flip(x, axes))
                seg_logits += torch.flip(seg_flipped, axes)
                cls_logits += cls_flipped
            seg_logits /= (len(axes_combinations) + 1)
            cls_logits /= (len(axes_combinations) + 1)
        ...
        try:
            ...
            with tqdm(desc=None, total=len(slicers), disable=not self.allow_tqdm) as pbar:
                while True:
                    ...
                    if 'cls_accum' not in locals():
                        cls_accum = None
                        cls_count = 0

                    if self.use_gaussian:
                        seg_pred *= gaussian
                    predicted_logits[sl] += seg_pred
                    n_predictions[sl[1:]] += gaussian

                    if cls_pred is not None:
                        cls_pred = cls_pred[0]  # remove batch dim
                        if cls_accum is None:
                            cls_accum = torch.zeros_like(cls_pred)
                        cls_accum += cls_pred
                        cls_count += 1
            ...
            # finalize classification and save
            if 'cls_accum' in locals() and cls_accum is not None and cls_count > 0:
                cls_mean = cls_accum / cls_count
                self._save_classification_output(cls_mean, results_device)
            # check for infs
    ...
    def _save_classification_output(self, cls_logits, device):
        cls_probs = F.softmax(cls_logits, dim=0)
        cls_pred = int(torch.argmax(cls_probs).item())
        save_path = os.path.join(self.output_folder if hasattr(self, 'output_folder') else '.', "classification_results.csv")
        os.makedirs(os.path.dirname(save_path), exist_ok=True)

        write_header = not os.path.exists(save_path)
        with open(save_path, "a") as f:
            if write_header:
                f.write("case,cls_pred\n")
            f.write(f"{self.current_case},{cls_pred}\n")
    ...
    def predict_from_files_sequential(self,
        ...
        for li, of, sps in zip(list_of_lists_or_source_folder, output_filename_truncated, seg_from_prev_stage_files):
            self.current_case = os.path.basename(of) if of else f"case_{len(ret)}"
            ...
            self._save_classification_output(prediction.mean(dim=0), self.device)
