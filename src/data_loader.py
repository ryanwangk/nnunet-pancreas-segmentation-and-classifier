...
class nnUNetDataLoader(DataLoader):
    def __init__(self,
        ...
        csv_path = os.environ.get("PCCLS_LABELS", None)
        if os.path.exists(csv_path):
            df = pd.read_csv(csv_path)
            df["Filename"] = df["Filename"].astype(str).str.replace(".nii.gz", "", regex=False)
            self.subtype_dict = dict(zip(df["Filename"], df["Subtype"]))
        else:
            self.subtype_dict = {}
    ...
    def generate_train_batch(self):
        ...
        for key in selected_keys:
            key_str = os.path.basename(str(key)).replace(".nii.gz", "")
            subtype = int(self.subtype_dict.get(key_str, 0))
            properties.append({"case_subtype": subtype})

        return {'data': data_all, 'target': seg_all, 'keys': selected_keys, 'properties': properties}
