...
class ResidualEncoderUNet(AbstractDynamicNetworkArchitectures):
    def __init__(
        ...
        is_3d = (conv_op is torch.nn.Conv3d)
        self.gap = nn.AdaptiveAvgPool3d(1) if is_3d else nn.AdaptiveAvgPool2d(1)
        self.classification_head = nn.Linear(features_per_stage[-1], 3)
